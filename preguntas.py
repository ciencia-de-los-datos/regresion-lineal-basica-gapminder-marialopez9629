...
def pregunta_02():
    ...
    print("{0:1.4f}".format(df.corr()["fertility"]["life"]))

    # Imprima la media de la columna `life` con 4 decimales.
    print("{0:1.4f}".format(df["life"].mean()))

    # Imprima el tipo de dato de la columna `fertility`.
    print(type(df["fertility"]))

    # Imprima la correlación entre las columnas `GDP` y `life` con 4 decimales.
    print("{0:1.4f}".format(df.corr()["GDP"]["life"]))
...
def pregunta_03():
    ...
    y_life_reshaped=y_life.reshape(-1,1)
    ...
    prediction_space = np.linspace(
        min(X_fertility_reshaped),
        max(X_fertility_reshaped),
    ).reshape(50, 1)
...
def pregunta_04():
    ...
    # Lea el archivo `gm_2008_region.csv` y asignelo al DataFrame `df`
    df = pd.read_csv("/content/regresion-lineal-basica-gapminder-marialopez9629/gm_2008_region.csv",sep=",")
